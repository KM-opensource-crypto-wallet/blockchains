...
export const EVMChain = (chain_name, _phrase, customRpcUrl) => {
  ...
  const createSendTransaction = async (wallet, tx) => {
    ...
    if (!customRpcUrl && premiumRpcUrls.length > 0) {
      const premium = await broadcastToAll(premiumRpcUrls);
      if (premium.response) {
  ...
  return {
    ...
    sendToken: async ({
      ...
      try {
        const tx = await retryFunc(async evmProvider => {
          ...
          return await contract.transfer.populateTransaction(
            to,
            value2,
            options,
          );
    ...
    createStaking: async ({
      ...
      const tx = await retryFunc(async evmProvider => {
        ...
        const builtTx = await EvmStakingProvider.createStaking({
          from,
          amountInWei,
          tokenBalance,
          contractAddress,
    ...
    deactivateStaking: async ({
      ...
      const tx = await retryFunc(async evmProvider => {
        ...
        const builtTx = await EvmStakingProvider.unStaking({
          from,
          contractAddress,
          stakingProviderName,
          walletSigner,
    ...
    stakingRewards: async ({
      ...
      const tx = await retryFunc(async evmProvider => {
        const finalEstimateGas = (BigInt(estimateGas) * 110n) / 100n;
        ...
        const unSignedTrx = await EvmStakingProvider.claimRewards({
          from,
          contractAddress,
          stakingProviderName,
          privateKey,
    ...
    readPermitAllowance: async ({
      ...
      retryFunc(async evmProvider => {
        ...
        [permit2Amount, permit2Expiration] = await permit2Contract.allowance(
          from,
          contractAddress,
          swapTo,
        );
    ...
    getEstimateFeeForPermitApprove: async ({
      ...
      retryFunc(async evmProvider => {
        ...
        let estimateGas;
        try {
          estimateGas = await permit2Contract.approve.estimateGas(
            contractAddress,
            swapTo,
            amountInWei,
        ...
        if (permit2Amount > 0n && permit2Amount < amountInWei) {
          estimateGas += 20000n;
        }
    ...
    approvePermit2: async ({
      ...
      const tx = await retryFunc(async evmProvider => {
        ...
        if (typeof finalEstimateGas !== 'bigint') {
          ...
          finalEstimateGas = await permit2Contract.approve.estimateGas(
            contractAddress,
            swapTo,
            amountInWei,
            BigInt(Math.floor(Date.now() / 1000) + 30 * 24 * 60 * 60), // 30 days
        ...
        const builtTx = await permit2Contract.approve.populateTransaction(
          contractAddress,
          swapTo,
          amountInWei,
          BigInt(Math.floor(Date.now() / 1000) + 30 * 24 * 60 * 60), // 30 days
      ...
      const permit2Hash = await createSendTransaction(
        new ethers.Wallet(privateKey),
        tx,
      );
      await EVMChain(chain_name, undefined, customRpcUrl).waitForConfirmation({
        transaction: permit2Hash,
        interval: 5000,
        retries: 15,
      });
      return {
